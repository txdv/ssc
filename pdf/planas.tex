...
\section{Plan}
\subsection{Goal}
Taskus isvardinti, uzdaviniai tam tikslui pasiekti.
Ka ismoks skaitytojas.
...
In addition the same measurements will be performed on the current 2.13 Scala compiler and the newest Dotty (Scala 3) compiler.
...
First, the prototype compiler will be written as fast as possible since it is a central artifact in this thesis.
...
Third, the metric measurement tool will be created which performs the compilation runs and measurements with various inputs and compilers and generates reproducible outputs.
