...
Once a minimum viable implementation will have been written and performance metrics collected to satisfy the research needs for this thesis and given enough time, additional features will be added allowing for better evaluation.

The core idea of the thesis is to attempt granularize the translation unit size in order to increase preformance of subsequent compilations of the same source file with minimal source modifications.

A translation unit is the entire input of source code that will be compiled and usually compilers tend to translate the the entire input although most of the compilation output from a subsequent run could be reused from the previous one.
