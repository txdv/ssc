...
When writing software developers tend to create only small changes in existing source files before running the compilation again for quick feedback loop of introduced errors.

The core idea of the thesis is to granularize the translation unit size order to increase preformance of subsequent compilations of the same source file with minimal source modifications to allow for an improvement in this feedback loop.

A translation unit is the entire input of source code that is compiled and is usually reprocessed in its entirity without consideration of previous outputs compilation outputs.
