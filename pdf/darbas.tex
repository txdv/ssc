...
Metodo našumo patobulinimas yra pamatuojamas skirtingiems išeities teksto variantams.
Taip pat našumas palyginamas su egzistuojančio kompilerio našumu.

Klasikiniai kompileriai išeities tekstą vienoje byloje traktuoja kaip nedalomą transliuojamą vienetą.
Pristatomas metodas bando pasinaudoti modernios programavimo kalbos struktūra ir susmulkinti transliuojamą vienetą į mažesnius bendram transliavimo našumui pagerinti.
...
Naujo kompilerio architektūra nuo pradžių yra pritaikoma metodui įgyvendinti.
Dėl sumažinto sudėtingumo metodas yra pritaikomas lengviau.

Sukurtam kompileriui atliekami įvairūs matavimai pritaikius bei nepritaikius naują metodą.
Geresniam greitaveikos pokyčio įvertinimui sukūriamas skirtingo dydžio ir sudėtingumo išeities tekstas.
Matavimo rezultatai rodo, jog priklausomai nuo išeities teksto sudėtingumo transliavimo greitaveika pagerėja nuo 5\% iki 30\%.
...
% todo: pamineti kad scala kompileri
...
% todo: not novel idea, present a similar implementation in an already existing compiler
...
% https://tex.stackexchange.com/questions/2311/add-a-legend-title-to-the-legend-box-with-pgfplots

\begin{figure}
\caption{Speedup with caching in milliseconds}
\label{graph:speed}
\begin{tikzpicture}
	\begin{axis}[
...
\begin{figure}
\caption{Speedup with caching in percent}
...
\begin{figure}
\caption{Runtime for 50 methods per class without and with caching}
\label{graph:phases}
\begin{tikzpicture}
    ...
    \begin{axis}[
	    ...
	    \legend{read, lex, parse, ast, class, write}
	    \input{./src/benchmark/stacked_plot_closed.tex}
    ...
    \begin{axis}[
	  ...
	  \input{./src/benchmark/stacked_plot.tex}
...
\begin{tikzpicture}
    ...
    \begin{axis}[
	    ...
	    \legend{read, lex, parse, ast, class, write}
	    \input{./src/benchmark/stacked_plot_cache_closed.tex}
    ...
    \begin{axis}[
	  ...
	  \input{./src/benchmark/stacked_plot_cache.tex}
...
% sunmeruoti ir padaryti dvi pagrindines isvadas
...
%todo: kodel appendixe nera lenteles
