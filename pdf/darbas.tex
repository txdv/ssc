...
Long compilation times of unit tests can make such an approach impractical.

If an action takes more than 10 seconds \cite{Usability} the user is likely to loose attention and starts to perform different tasks.
The norm for compilers it to not show any kind of progress when compiling a single translation units.
This exacerbates the perceived \cite{Usability} time it takes to complete the process for the user.

%These kind of interruption during software development

\section{Benchmarking}
In order to verify.
%Some text about benchmarking.
