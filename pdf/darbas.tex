...
\section{Compiler}
At the start of the bachelor thesis a thorough investigation was made to analyze how the current Scala compiler works \cite{ScalaGithub} and if it is feasible to apply the targeted optimizations.
...
Without the help of a mentor with high proefficieny in the Scala codebase and taking the scope of the bachelor thesis into account, it was deemded too complicated to undergo such a task.

Instead of adjusting an already existing compiler, a compiler was written from scratch with an iterative approach.
Writing a minimal compiler allows one to get rid of all of the baggage that comes up with a big and mature compiler and it allows the developer to just focus on the actual task.
